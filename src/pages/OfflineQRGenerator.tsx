...
const OfflineQRGenerator: React.FC = () => {
  ...
  const [showBgPicker, setShowBgPicker] = useState(false);
  const [showFgPicker, setShowFgPicker] = useState(false);
  const [showPaddingColorPicker, setShowPaddingColorPicker] = useState(false);
  const [logoImage, setLogoImage] = useState<string | null>(null);
  const [showDownloadOptions, setShowDownloadOptions] = useState(false);
  const [isGenerating, setIsGenerating] = useState(false);
  const [isGenerated, setIsGenerated] = useState(false);
  const [activeTab, setActiveTab] = useState('basic');
  const [padding, setPadding] = useState(10); // Padding in pixels
  ...
  const downloadQRCode = (format: 'png' | 'svg') => {
    ...
    tempCtx.fillStyle = paddingColor;
    tempCtx.fillRect(0, 0, paddedSize, paddedSize);
    
    // Draw the QR code in the center
    tempCtx.drawImage(canvas, padding, padding, qrSize, qrSize);
    ...
    if (format === 'png') {
      ...
      const svgData = `
        <svg xmlns="http://www.w3.org/2000/svg" width="${paddedSize}" height="${paddedSize}">
          ...
          <foreignObject x="${padding}" y="${padding}" width="${qrSize}" height="${qrSize}">
            <div xmlns="http://www.w3.org/1999/xhtml">
              <img src="${tempCanvas.toDataURL('image/png')}" width="${qrSize}" height="${qrSize}" />
            </div>
          </foreignObject>
        </svg>
      `;
  ...
  const handleRGBChange = (color: string, setColor: (color: string) => void) => {
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full"
                        />
                        <span className="text-sm text-gray-600">{padding}px</span>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <div 
                            ...
                            onClick={() => setShowPaddingColorPicker(!showPaddingColorPicker)}
                          />
                        </div>
                        
                        {showPaddingColorPicker && (
                          <div className="relative z-10">
                            <div 
                              className="fixed inset-0" 
                              onClick={() => setShowPaddingColorPicker(false)}
                            />
                            <div className="absolute right-0">
